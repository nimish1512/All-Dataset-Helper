...
class CKLoader:
	...
	def format_data(self,train_test_split=0.8, normalized_test_split=False, shuffle=False, channels=1, img_dims=[32,32], preprocess=[], batch_size=128):
		try:
			train_images = list()
			train_labels = list()
			test_images = list()
...
with tf.Session() as sess:
	x = CKLoader()
	x.format_data(batch_size=128)
	#sess.run(x.trainer_init)
	i=1
	while True:
		try:
